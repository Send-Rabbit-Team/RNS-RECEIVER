...
public class RabbitmqHeader {
    private static final String KEY_WORD_WORK_QUEUE = "work";
    ...
    public int getFailedRetryCount() {
        ...
        for (RabbitmqHeaderXDeath xDeath : xDeaths) {
            if (xDeath.getExchange().toLowerCase().endsWith(KEY_WORD_WORK_QUEUE)
                    && xDeath.getQueue().toLowerCase().endsWith(KEY_WORD_WORK_QUEUE))
